...
import { DeleteIcon, AddIcon } from '@iso/config/icon.config';
...
export default function Invoices() {
  ...
  const menuClicked = (
    ...
    >
      <MenuItem
        key="1"
        style={{ borderBottom: '1px solid #dadada', color: '#6a6a6a' }}
      >
        Tìm kiếm
      </MenuItem>
      <MenuItem key="2" style={{ color: '#6a6a6a' }}>
        Xóa theo danh sách đã chọn
      </MenuItem>
    </DropdownMenu>
  );
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.manageVehicle" />
      </PageHeader>
      <Box>
          ...
          <Link to={`${match.path}/${getnewInvoiceId()}`}>
            ...
            >
              ...
              <span style={{ lineHeight: '200' }}>
                <IntlMessages id="sidebar.addVehicle" />
              </span>
            </Button>
          </Link>
        </div>
