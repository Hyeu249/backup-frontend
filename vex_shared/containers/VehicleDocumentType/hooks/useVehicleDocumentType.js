...
const useVehicleDocumentType = () => {
  ...
  const vehicleDocumentTypes = useSelector(
    state => state.vehicleDocumentType.vehicleDocumentTypes
  );
    ...
    selections: [
      {
        ...
        onSelect: () =>
          setSelected(
            vehicleDocumentTypes.map(
              vehicleDocumentTypes => vehicleDocumentTypes.key
            )
          ),
      },
      {
  ...
  const columns = [
    {
      title: intl
        .formatMessage({ id: 'text.vehicleDocumentTypeName' })
        ?.split('(')[0],
      dataIndex: 'name',
      rowKey: 'name',
      width: 'auto',
    ...
    {
      title: intl
        .formatMessage({ id: 'text.vehicleDocumentTypeDescription' })
        ?.split('(')[0],
    ...
    },
    {
      ...
      rowKey: 'view',
      width: '10%',
      render: (text, vehicleDocumentType) => (
        <div className="isoBtnView">
          ...
          <Button
            ...
            onClick={() => {
              dispatch(
                setViewedVehicleDocumentType({
                  vehicleDocumentTypeID: vehicleDocumentType.id,
                  setOpen: setEditOpen,
                })
              );
            }}
          ...
          <Button
            ...
            onClick={() => {
              console.log(
                'deleted vehicleDocumentType_id: ',
                vehicleDocumentType.id
              );
              dispatch(deleteVehicleDocumentType(vehicleDocumentType.id));
            }}
          >
            <DeleteIcon size={18} />
          </Button>
...
export default useVehicleDocumentType;
