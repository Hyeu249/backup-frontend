...
import useVehicleDocumentType from './hooks/useVehicleDocumentType';
// import CardWrapper, { Box } from './VehicleDocumentType.styles';
...
export default function VehicleDocumentType() {
  const {
    createOpen,
    setCreateOpen,
    editOpen,
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.vehicleDocumentType" />
      </PageHeader>
      <Box>
        <div
        ...
        >
          ...
          >
            ...
            <span style={{ lineHeight: '200' }}>
              <IntlMessages id="text.addVehicleDocumentType" />
            </span>
          </Button>
        </div>

        <CardWrapper title="Vehicle Document Type">
          {vehicleDocumentTypes.length === 0 ? (
            <HelperText text="No Vehicle Document Type" />
          ) : (
            <div className="isoTable">
              ...
              >
                <TableWrapper
                  ...
                  dataSource={vehicleDocumentTypes}
                  columns={columns}
                  pagination={false}
                  className="ListTable"
                />
