...
import EditSupplierQuote from './EditSupplierQuote/EditSupplierQuote';
...
const EditOverall = () => {
  ...
  const viewedSupplierQuote = cloneDeep(
    useSelector(state => state.supplierQuote.viewedSupplierQuote)
  );
  const isHaveViewedSupplierQuote = Object.keys(viewedSupplierQuote).length > 0;

  useEffect(() => {
    dispatch(initEditData({ supplierQuoteID: id }));
    return () => {
      dispatch(removeEditData());
    };
  }, []);

  return isHaveViewedSupplierQuote ? (
    <div>
      <Header />
      <Content>
        <LeftContent>
