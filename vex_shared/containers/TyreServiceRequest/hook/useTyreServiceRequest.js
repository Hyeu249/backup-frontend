...
const useManageTyreServiceRequest = () => {
  const intl = useIntl();
  const dispatch = useDispatch();
  const match = useRouteMatch();

  const { initData, removeInitData, deleteTyreServiceRequest } = actions;

  const tyreServiceRequests = useSelector(
    state => state.tyreServiceRequest.tyreServiceRequests
  );
    ...
    selections: [
      {
        ...
        onSelect: () =>
          setSelected(
            tyreServiceRequests.map(
              tyreServiceRequests => tyreServiceRequests.key
            )
          ),
      },
      {
  ...
  const columns = [
    {
      title: intl
        .formatMessage({ id: 'text.tyreServiceRequestPlannedServiceTime' })
        ?.split('(')[0],
    ...
    {
      title: intl
        .formatMessage({ id: 'text.tyreServiceRequestActualServiceTime' })
        ?.split('(')[0],
...

    {
      ...
      rowKey: 'view',
      width: '10%',
      render: (text, tyreServiceRequest) => (
        <div className="isoBtnView">
          {/* view button */}
          <Link to={`${match.path}/${tyreServiceRequest.id}`}>
          ...
          <Button
            ...
            onClick={() => {
              console.log(
                'deleted tyreServiceRequest_id: ',
                tyreServiceRequest.id
              );
              dispatch(deleteTyreServiceRequest(tyreServiceRequest.id));
            }}
          >
            <DeleteIcon size={18} />
          </Button>
...
export default useManageTyreServiceRequest;
