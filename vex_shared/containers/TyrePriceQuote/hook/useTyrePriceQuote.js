...
const useManageTyrePriceQuote = () => {
  ...
  const tyrePriceQuotes = useSelector(
    state => state.tyrePriceQuote.tyrePriceQuotes
  );
    ...
    selections: [
      {
        ...
        onSelect: () =>
          setSelected(
            tyrePriceQuotes.map(tyrePriceQuotes => tyrePriceQuotes.key)
          ),
      },
      {
        key: 'no-data',
  ...
  const columns = [
    {
      title: intl
        .formatMessage({ id: 'text.tyrePriceQuoteTyrePrice' })
        ?.split('(')[0],
      dataIndex: 'tyre_price',
      rowKey: 'tyre_price',
      width: 'auto',
    ...
    {
      title: intl
        .formatMessage({ id: 'text.tyrePriceQuoteLaborCost' })
        ?.split('(')[0],
    ...
    {
      title: intl
        .formatMessage({ id: 'text.tyrePriceQuoteValidFrom' })
        ?.split('(')[0],
    ...
    {
      title: intl
        .formatMessage({ id: 'text.tyrePriceQuoteValidUntil' })
        ?.split('(')[0],
    ...
    },
    {
      ...
      rowKey: 'view',
      width: '10%',
      render: (text, tyrePriceQuote) => (
        <div className="isoBtnView">
          {/* view button */}
          <Link to={`${match.path}/${tyrePriceQuote.id}`}>
          ...
          <Button
            className="DltBtn"
            onClick={() => {
              console.log('deleted tyrePriceQuote_id: ', tyrePriceQuote.id);
              dispatch(deleteTyrePriceQuote(tyrePriceQuote.id));
            }}
          >
            <DeleteIcon size={18} />
          </Button>
...
export default useManageTyrePriceQuote;
