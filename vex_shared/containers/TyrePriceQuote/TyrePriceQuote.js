...
import useTyrePriceQuote from './hook/useTyrePriceQuote';
import CreateModal from './CreateModal/CreateModal';

export default function TyrePriceQuote() {
  const {
    createOpen,
    setCreateOpen,
    tyrePriceQuotes,
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.tyrePriceQuote" />
      </PageHeader>
      <Box>
        <div
        ...
        >
          ...
          >
            ...
            <span style={{ lineHeight: '200' }}>
              <IntlMessages id="text.addTyrePriceQuote" />
            </span>
          </Button>
        </div>

        <CardWrapper title="Tyre Price Quote">
          {tyrePriceQuotes?.length === 0 ? (
            <HelperText text="No Tyre Price Quotes" />
          ) : (
