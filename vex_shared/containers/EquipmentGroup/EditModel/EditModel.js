...
const EditModal = ({ open, setOpen }) => {
  ...
  const { updateEquipmentGroup, removeViewedEquipmentGroup } =
    equipmentGroupActions;
  const [isAllowEdit, setIsAllowEdit] = useState(false);

  const { rawFields } = useRawFields();
  const viewedEquipmentGroup = useSelector(
    state => state.equipmentGroup.viewedEquipmentGroup
  );
  //state

  function ConfirmEditEquipmentGroupHandle() {
    const equipmentGroupData = this;

    confirm({
      title: intl.formatMessage({
        id: 'text.confirm.equipmentGroupConfirmEdit',
      }),
      icon: <ExclamationCircleFilled />,
      content: intl.formatMessage({
        id: 'text.confirm.equipmentGroupDescriptionEdit',
      }),
      okText: intl.formatMessage({ id: 'text.ok' }),
      cancelText: intl.formatMessage({ id: 'text.cancel' }),
      onOk() {
        dispatch(updateEquipmentGroup(equipmentGroupData));
      },
      onCancel() {
        setOpen(true);
      },
  ...
  const editTitle = intl.formatMessage({ id: 'text.editEquipmentGroup' });
  ...
  return (
    <React.Fragment>
      <ModalTemplate
        ...
        confirmHandle={ConfirmEditEquipmentGroupHandle}
        rawFields={rawFields}
        //props for edit
        parentComponent={EDIT_MODAL}
        setIsAllowEdit={setIsAllowEdit}
        viewedData={viewedEquipmentGroup}
        removeViewedData={removeViewedEquipmentGroup}
      />
    </React.Fragment>
  );
};
