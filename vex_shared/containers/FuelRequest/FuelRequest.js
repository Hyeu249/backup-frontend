...
import useFuelRequest from './hook/useFuelRequest';
...
export default function FuelRequest() {
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.fuelRequest" />
      </PageHeader>
      <Box>
        <div
        ...
        >
          ...
          >
            ...
            <span style={{ lineHeight: '200' }}>
              <IntlMessages id="sidebar.addFuelRequest" />
            </span>
          </Button>
        </div>

        <CardWrapper title="FuelRequest">
          {fuelRequests?.length === 0 ? (
            <HelperText text="No FuelRequests" />
          ) : (
