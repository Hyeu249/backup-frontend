...
const useManageRole = () => {
  ...
  const { initData, removeInitData, deleteRole } = actions;
    ...
    selections: [
      {
        key: 'all-data',
        text: 'Select All',
        onSelect: () => setSelected(roles.map(roles => roles.key)),
      },
      {
        key: 'no-data',
        text: 'Unselect all',
  ...
  const columns = [
    {
      title: intl.formatMessage({ id: 'text.authRole' })?.split('(')[0],
      dataIndex: 'label',
      rowKey: 'label',
      width: 'auto',
      ...
      render: (text, role) => (
        <div className="isoBtnView">
          ...
          <Button
            className="DltBtn"
            onClick={() => {
              console.log('deleted role_id: ', role.id);
