...
const useFuel = () => {
  ...
  const { initData, removeInitData, setViewedFuel, deleteFuel } = fuelActions;
    ...
    selections: [
      {
        key: 'all-data',
        text: 'Select All',
        onSelect: () => setSelected(fuels.map(fuels => fuels.key)),
      },
      {
        key: 'no-data',
        text: 'Unselect all',
  ...
  const columns = [
    {
      title: intl.formatMessage({ id: 'text.fuelName' })?.split('(')[0],
      dataIndex: 'name',
      rowKey: 'name',
      width: 'auto',
      ...
      render: (text, fuel) => (
        <div className="isoBtnView">
          ...
          <Button
            className="DltBtn"
            onClick={() => {
              console.log('deleted fuel_id: ', fuel.id);
              dispatch(deleteFuel(fuel.id));
            }}
          >
            <DeleteIcon size={18} />
          </Button>
