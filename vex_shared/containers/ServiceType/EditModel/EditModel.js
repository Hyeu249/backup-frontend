...
const EditModal = ({ open, setOpen }) => {
  ...
  const { updateServiceType, removeViewedServiceType } = serviceTypeActions;
  const [isAllowEdit, setIsAllowEdit] = useState(false);

  const { rawFields } = useRawFields();
  const viewedServiceType = useSelector(
    state => state.serviceType.viewedServiceType
  );
  //state

  function ConfirmEditServiceTypeHandle() {
    const serviceTypeData = this;

    confirm({
      title: intl.formatMessage({
        id: 'text.confirm.serviceTypeConfirmEdit',
      }),
      icon: <ExclamationCircleFilled />,
      content: intl.formatMessage({
        id: 'text.confirm.serviceTypeDescriptionEdit',
      }),
      okText: intl.formatMessage({ id: 'text.ok' }),
      cancelText: intl.formatMessage({ id: 'text.cancel' }),
      onOk() {
        dispatch(updateServiceType(serviceTypeData));
      },
      onCancel() {
        setOpen(true);
      },
  ...
  const editTitle = intl.formatMessage({ id: 'text.editServiceType' });
  ...
  return (
    <React.Fragment>
      <ModalTemplate
        ...
        confirmHandle={ConfirmEditServiceTypeHandle}
        rawFields={rawFields}
        //props for edit
        parentComponent={EDIT_MODAL}
        setIsAllowEdit={setIsAllowEdit}
        viewedData={viewedServiceType}
        removeViewedData={removeViewedServiceType}
      />
    </React.Fragment>
  );
};
