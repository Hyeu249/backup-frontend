...
import EditRoleOfUser from './EditRoleOfUser/EditRoleOfUser';
...
const EditOverall = () => {
  ...
  const viewedRoleOfUser = cloneDeep(
    useSelector(state => state.roleOfUser.viewedRoleOfUser)
  );
  const isHaveViewedRoleOfUser = Object.keys(viewedRoleOfUser).length > 0;

  useEffect(() => {
    dispatch(initEditData({ roleOfUserID: id }));
    return () => {
      dispatch(removeEditData());
    };
  }, []);

  return isHaveViewedRoleOfUser ? (
    <div>
      <Header />
      <Content>
        <LeftContent>
