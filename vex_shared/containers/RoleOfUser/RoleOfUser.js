...
import useRoleOfUser from './hook/useRoleOfUser';
import CreateModal from './CreateModal/CreateModal';

export default function RoleOfUser() {
  const {
    createOpen,
    setCreateOpen,
    roleOfUsers,
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.roleOfUser" />
      </PageHeader>
      <Box>
        <div
        ...
        >
          ...
          >
            ...
            <span style={{ lineHeight: '200' }}>
              <IntlMessages id="sidebar.addRoleOfUser" />
            </span>
          </Button>
        </div>

        <CardWrapper title="RoleOfUser">
          {roleOfUsers?.length === 0 ? (
            <HelperText text="No Role Of Users" />
          ) : (
