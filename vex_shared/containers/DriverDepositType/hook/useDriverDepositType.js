...
const useDriverDepositType = () => {
  ...
  const driverDepositTypes = useSelector(
    state => state.driverDepositType.driverDepositTypes
  );
    ...
    selections: [
      {
        ...
        onSelect: () =>
          setSelected(
            driverDepositTypes.map(driverDepositTypes => driverDepositTypes.key)
          ),
      },
      {
        key: 'no-data',
  ...
  const columns = [
    {
      title: intl
        .formatMessage({ id: 'text.driverDepositTypeName' })
        ?.split('(')[0],
      dataIndex: 'name',
      rowKey: 'name',
      width: 'auto',
    ...
    {
      title: intl
        .formatMessage({ id: 'text.driverDepositTypeDescription' })
        ?.split('(')[0],
      ...
      render: (text, driverDepositType) => (
        <div className="isoBtnView">
          ...
          <Button
            ...
            className="ViewBtn"
            onClick={() => {
              dispatch(setViewedDriverDepositType(driverDepositType));
              setEditOpen(true);
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              console.log(
                'deleted driverDepositType_id: ',
                driverDepositType.id
              );
              dispatch(deleteDriverDepositType(driverDepositType.id));
            }}
          >
            <DeleteIcon size={18} />
          </Button>
...
export default useDriverDepositType;
